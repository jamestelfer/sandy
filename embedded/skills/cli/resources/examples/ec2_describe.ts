...
async function* findInstances(tag: string, value: string): AsyncGenerator<Instance> {
  ...
  do {
    ...
    progress(`fetched page (${nextToken ? "more pages" : "done"})`)
  } while (nextToken)
}
...
for await (const instance of findInstances(tagName, tagValue)) {
  const info: InstanceInfo = search(
    instance,
    `{
